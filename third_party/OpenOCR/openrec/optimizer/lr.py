...
class OneCycleLR(object):
    ...
    def __init__(self,
        ...
        self.lr = lr
        computed_total_steps = int(epochs * step_each_epoch)
        self.total_steps = int(
            total_steps if total_steps is not None else computed_total_steps)
        if self.total_steps < 2:
            raise ValueError(
                f'OneCycleLR total_steps must be at least 2, got {self.total_steps}')
        if warmup_steps is None:
            warmup_steps = float(warmup_epoch) * step_each_epoch
        self.warmup_steps = float(warmup_steps)
        if not 0 < self.warmup_steps < self.total_steps:
            raise ValueError(
                'OneCycleLR warmup_steps must be in (0, total_steps), got '
                f'{self.warmup_steps}/{self.total_steps}')
        self.pct_start = self.warmup_steps / self.total_steps

...
class Trainer(object):
    ...
    def train(self):
        ...
        stop_after_epoch = int(
            self.cfg['Global'].get('stop_after_epoch', epoch_num))
        if stop_after_epoch < 1 or stop_after_epoch > epoch_num:
            raise ValueError(
                'Global.stop_after_epoch must be in [1, epoch_num], got '
                f'{stop_after_epoch} for epoch_num={epoch_num}')
        max_optimizer_steps = self.cfg['Global'].get('max_optimizer_steps')
        if max_optimizer_steps is not None:
            max_optimizer_steps = int(max_optimizer_steps)
            if max_optimizer_steps < 1:
                raise ValueError(
                    'Global.max_optimizer_steps must be positive, got '
                    f'{max_optimizer_steps}')
        planned_batches = (
            max_optimizer_steps
            if max_optimizer_steps is not None
            else epoch_num * len(self.train_dataloader)
        )
        planned_optimizer_updates = math.ceil(
            planned_batches / self.accumulation_steps
        )
        ...
        for epoch in range(start_epoch, epoch_num + 1):
            ...
            observed_epoch_batches = 0
            for idx, batch in enumerate(self.train_dataloader):
                observed_epoch_batches += 1
                if self.cfg['Global'].get('resume_from_iter',
                                          False):  # for unirec resume training
                ...
                # build_dataloader uses pinned host memory for CUDA runs;
                # non_blocking keeps the host-to-device copy from needlessly
                # serializing the next compute step.
                batch_tensor = [
                    t.to(self.device, non_blocking=True) for t in batch
                ]
                batch_numpy = [t.numpy() for t in batch]
                train_reader_cost += time.time() - reader_start
                if hasattr(self.loss_class, 'set_training_progress'):
                    self.loss_class.set_training_progress(
                        global_step // self.accumulation_steps,
                ...
                if self.scaler:
                    ...
                    if not torch.isfinite(raw_loss).all():
                        raise FloatingPointError(
                            ...
                            f'global_step={global_step}: {raw_loss.detach().cpu()}'
                        )
                    loss['loss'] = raw_loss / self.accumulation_steps
                    loss['loss'].backward()
                    if (global_step + 1) % self.accumulation_steps == 0:
                        if self.grad_clip_val > 0:
                            torch.nn.utils.clip_grad_norm_(
                ...
                try:
                    ...
                    if self.cfg['Global'].get('strict_lr_scheduler', False):
                ...
                if (
                    ...
                    if global_step != max_optimizer_steps:
                        raise RuntimeError(
                            'Optimizer-step boundary was crossed: '
                            f'{global_step} != {max_optimizer_steps}')
                    if is_main_process():
                        save_ckpt(
                            self.model,
                            self.cfg,
                        ...
                        self.logger.info(
                            'Reached controlled optimizer-step boundary at '
                            f'{global_step}; exiting after checkpoint save.')
                    reached_step_boundary = True
                    break

            expected_epoch_batches = len(self.train_dataloader)
            if (
                self.cfg['Global'].get('strict_epoch_length', False)
                and not reached_step_boundary
                and observed_epoch_batches != expected_epoch_batches
            ):
                raise RuntimeError(
                    'Train dataloader length contract violated at '
                    f'epoch={epoch}: observed={observed_epoch_batches}, '
                    f'len(dataloader)={expected_epoch_batches}. The LR '
                    'scheduler horizon would be invalid.')

            if reached_step_boundary:
                break
            ...
            if epoch >= stop_after_epoch:
                if is_main_process():
                    self.logger.info(
                        'Reached controlled segment boundary at epoch '
                        f'{epoch}/{epoch_num}; exiting after checkpoint save.')
                break

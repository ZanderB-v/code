...
def build_dataloader(config, mode, logger, seed=None, epoch=1, task='rec'):
    ...
    # P1/MSR training repeatedly transfers variable-width tensors from CPU to
    # CUDA.  Pinned memory plus persistent workers removes avoidable transfer
    # stalls, while keeping the knobs overridable in a config.
    device_name = str(config.get('Global', {}).get('device', '')).lower()
    pin_memory = loader_config.get(
        'pin_memory', device_name in {'gpu', 'cuda'})
    persistent_workers = loader_config.get(
        'persistent_workers', num_workers > 0 and mode == 'Train')
    prefetch_factor = loader_config.get('prefetch_factor', 2)
    logger.info(
        f'{mode} DataLoader: workers={num_workers}, '
    ...
    if module_name == 'CMERWebDataSet':
        ...
        if len(data_loader) == 0:
            message = (
                ...
                f'batch_sampler_len={len(batch_sampler) if batch_sampler is not None else "none"}, '

...
SESSION="${SESSION:-m3_sldr_target_only_pilot_v1}"
GPU_ID="${GPU_ID:-0}"
...
for required in "$M3_S50_CHECKPOINT" "$PROFILE" "$GATE" \
...
state_dir="${STATE_DIR:-$ROOT/04_model_training/logs/m3_sldr_target_only_pilot_v1_state}"
console_log="$ROOT/04_model_training/logs/m3_sldr_target_only_pilot_v1_$stamp.log"
mkdir -p "$state_dir"
command=(
  "$PYTHON" "$ROOT/scripts/svtrv2/run_m3_sldr_pilot.py"
  --root "$ROOT"
  --log-dir "$state_dir"
...
echo "Physical GPU: $GPU_ID only; controlled global batch size: $BATCH_SIZE"
echo "Console log: $console_log"
echo "Audited resume state: $state_dir"
echo "Attach: tmux attach -t $SESSION"
echo "Pilot: target-only M3+SLDR; Clean Dev V4 selection; Test forbidden."

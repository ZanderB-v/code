...
if [[ "${1:-}" == "--worker" ]]; then
  ...
  python scripts/svtrv2/test_dual_order_components.py --root "$ROOT"
  python scripts/svtrv2/test_semantic_direction_v2_workflow.py --root "$ROOT"
  python -m torch.distributed.run --nproc_per_node=2 --master_port=29939 \
    scripts/svtrv2/test_semantic_direction.py --root "$ROOT" --ddp
  python scripts/svtrv2/run_semantic_direction_v2.py \
...
mkdir -p "$LOG_DIR"
printf -v WORKER '%q ' bash "$ROOT/scripts/svtrv2/run_semantic_direction_v2_tmux.sh" --worker "$@"
tmux new-session -d -s "$SESSION"
tmux set-window-option -t "$SESSION":0 remain-on-exit on
printf -v COMMAND 'set -o pipefail; %s 2>&1 | tee -a %q; rc=${PIPESTATUS[0]}; printf "DIRECTION_V2_EXIT_CODE=%%s\\n" "$rc"; exit "$rc"' \
...
echo "Resume with the same command; do not add --replace."

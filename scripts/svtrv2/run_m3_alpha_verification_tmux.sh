...
if [[ "${1:-}" == "--worker" ]]; then
  ...
  python scripts/svtrv2/test_dual_order_components.py --root "$ROOT" \
    --output "$LOG_DIR/dual_order_component_invariants.json"
  python scripts/svtrv2/test_m3_alpha_verification.py --root "$ROOT"
  python scripts/svtrv2/run_m3_alpha_verification.py \
...
mkdir -p "$LOG_DIR"
printf -v WORKER '%q ' bash "$ROOT/scripts/svtrv2/run_m3_alpha_verification_tmux.sh" --worker "$@"
tmux new-session -d -s "$SESSION"
tmux set-window-option -t "$SESSION":0 remain-on-exit on
printf -v COMMAND 'set -o pipefail; %s 2>&1 | tee -a %q; rc=${PIPESTATUS[0]}; printf "M3_ALPHA_VERIFICATION_EXIT_CODE=%%s\n" "$rc"; exit "$rc"' \
...
echo "Resume with the same command; never add --replace."

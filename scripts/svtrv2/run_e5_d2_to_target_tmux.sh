...
CONFIG_PATH="$ROOT/04_model_training/configs/svtrv2_s_e5_d2_to_target.yml"
E1_LABELS_DIR="$ROOT/04_model_training/datasets/e1_target_only/labels"
TARGET_DIR="$ROOT/01_data_preparation/real_line_dataset_eval_reviewed"
D2_CHECKPOINT="$ROOT/04_model_training/runs/svtrv2_s_d2_synth50k/best_real_dev_macro_cer.pth"
RUN_MODEL_DIR="$ROOT/04_model_training/runs/svtrv2_s_e5_d2_to_target"
EVAL_ROOT="$ROOT/04_model_training/eval_reports"
LOG_DIR="$ROOT/04_model_training/logs/e5_d2_to_target_$(date +%Y%m%d_%H%M%S)"

TRAIN_SESSION="${TRAIN_SESSION:-e5_d2_to_target_train}"
WATCH_SESSION="${WATCH_SESSION:-e5_d2_to_target_watch_eval}"
...
# Keep E1's learning rate for the controlled E1/E5 comparison.
LR="${LR:-0.00005}"

usage() {
  cat <<EOF
Usage:
  bash scripts/svtrv2/run_e5_d2_to_target_tmux.sh [--replace]

E5 initializes SVTRv2-S from the D2 synthetic-only best checkpoint and then
fine-tunes on exactly the same reviewed target-domain train labels as E1.
...
for session in "$TRAIN_SESSION" "$WATCH_SESSION"; do
  if tmux has-session -t "$session" 2>/dev/null; then
    if [[ "$REPLACE" == "1" ]]; then
      ...
      echo "Use --replace to kill and restart E5 sessions." >&2
      exit 1
    fi
  fi
done

if [[ ! -s "$D2_CHECKPOINT" ]]; then
  echo "Missing D2 best checkpoint: $D2_CHECKPOINT" >&2
  echo "Restore/sync it before starting E5." >&2
  exit 1
fi
...
if [[ "$REPLACE" == "1" ]]; then
  rm -rf \
    "$RUN_MODEL_DIR" \
    "$EVAL_ROOT"/e5_d2_to_target_dev_epoch_* \
    "$EVAL_ROOT/e5_d2_to_target_best_target_dev" \
    "$EVAL_ROOT/e5_d2_to_target_best_target_test" \
    "$EVAL_ROOT/e5_d2_to_target_final_summary.json"
fi
...
echo "[1/3] Preparing controlled E5 config..."
python scripts/svtrv2/prepare_e5_d2_to_target.py \
  --root "$ROOT" \
  --d2-checkpoint "$D2_CHECKPOINT" \
  --max-epoch "$MAX_EPOCH" \
...
echo "Initial checkpoint: $D2_CHECKPOINT"
...
for EPOCH in \$(seq "$EVAL_EVERY" "$EVAL_EVERY" "$MAX_EPOCH"); do
  ...
  OUT_DIR="$EVAL_ROOT/e5_d2_to_target_dev_epoch_\${EPOCH_PAD}"
...
if [[ ! -s "\$BEST_CKPT" ]]; then
  echo "No best E5 checkpoint was selected." >&2
  exit 1
fi

echo "Evaluating selected E5 checkpoint on target-domain dev..."
rm -rf "$EVAL_ROOT/e5_d2_to_target_best_target_dev"
python scripts/svtrv2/evaluate_d2_checkpoint.py \
  --root "$ROOT" \
  --config "$CONFIG_PATH" \
  ...
  --labels-dir "$E1_LABELS_DIR" \
  --label-prefix target \
  --output-dir "$EVAL_ROOT/e5_d2_to_target_best_target_dev" \
  --device-id "$EVAL_GPU" \
  --batch-size 32 \
  --require-python-bidi \
  2>&1 | tee "$LOG_DIR/final_dev_eval.log"

echo "Evaluating selected E5 checkpoint once on target-domain test..."
rm -rf "$EVAL_ROOT/e5_d2_to_target_best_target_test"
python scripts/svtrv2/evaluate_d2_checkpoint.py \
  --root "$ROOT" \
  --config "$CONFIG_PATH" \
  --checkpoint "\$BEST_CKPT" \
  ...
  --labels-dir "$E1_LABELS_DIR" \
  --label-prefix target \
  --output-dir "$EVAL_ROOT/e5_d2_to_target_best_target_test" \
  --device-id "$EVAL_GPU" \
  --batch-size 32 \
  --require-python-bidi \
  2>&1 | tee "$LOG_DIR/final_test_eval.log"

python -c "import json, pathlib; out=pathlib.Path('$EVAL_ROOT/e5_d2_to_target_final_summary.json'); best=json.load(open('$RUN_MODEL_DIR/best_target_dev_macro_cer.json', encoding='utf-8')); dev=json.load(open('$EVAL_ROOT/e5_d2_to_target_best_target_dev/metrics_macro_summary.json', encoding='utf-8')); test=json.load(open('$EVAL_ROOT/e5_d2_to_target_best_target_test/metrics_macro_summary.json', encoding='utf-8')); prep=json.load(open('$ROOT/04_model_training/datasets/e5_d2_to_target/prepare_summary.json', encoding='utf-8')); data={'experiment':'E5_D2_synthetic_pretrain_then_target_finetune','best':best,'dev':dev,'test':test,'preparation':prep,'logs':'$LOG_DIR','run_dir':'$RUN_MODEL_DIR'}; out.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding='utf-8'); print(json.dumps(data, ensure_ascii=False, indent=2))" \
  2>&1 | tee "$LOG_DIR/final_summary.log"

echo "DONE"
echo "Best checkpoint: \$BEST_CKPT"
echo "Final summary: $EVAL_ROOT/e5_d2_to_target_final_summary.json"
EOF
chmod +x "$LOG_DIR/watch_eval.sh"

echo "[2/3] Starting dual-GPU DDP E5 training: $TRAIN_SESSION"
tmux new-session -d -s "$TRAIN_SESSION" \
  "bash '$LOG_DIR/train_ddp.sh'"

echo "[3/3] Starting E5 target-dev evaluator: $WATCH_SESSION"
...
echo "Logs: $LOG_DIR"
echo "Initial D2 checkpoint: $D2_CHECKPOINT"
echo "Best E5 checkpoint: $RUN_MODEL_DIR/best_target_dev_macro_cer.pth"
echo "Final summary: $EVAL_ROOT/e5_d2_to_target_final_summary.json"

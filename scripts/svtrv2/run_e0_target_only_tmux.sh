...
CONFIG_PATH="$ROOT/04_model_training/configs/svtrv2_s_e0_random_target_only.yml"
DATASET_DIR="$ROOT/04_model_training/datasets/e0_random_target_only"
LABELS_DIR="$DATASET_DIR/labels"
TARGET_DIR="$ROOT/01_data_preparation/real_line_dataset_eval_reviewed"
RUN_MODEL_DIR="$ROOT/04_model_training/runs/svtrv2_s_e0_random_target_only"
EVAL_ROOT="$ROOT/04_model_training/eval_reports"
LOG_DIR="$ROOT/04_model_training/logs/e0_random_target_only_$(date +%Y%m%d_%H%M%S)"

TRAIN_SESSION="${TRAIN_SESSION:-e0_random_target_train}"
WATCH_SESSION="${WATCH_SESSION:-e0_random_target_watch_eval}"
DDP_GPUS="${DDP_GPUS:-0,1}"
EVAL_GPU="${EVAL_GPU:-1}"
MASTER_PORT="${MASTER_PORT:-29533}"
MAX_EPOCH="${MAX_EPOCH:-50}"
EVAL_EVERY="${EVAL_EVERY:-2}"
PATIENCE_EVALS="${PATIENCE_EVALS:-8}"
MIN_EPOCH="${MIN_EPOCH:-20}"
...
usage() {
  cat <<EOF
Usage:
  bash scripts/svtrv2/run_e0_target_only_tmux.sh [--replace]

E0 trains the same SVTRv2-S-RCTC target-domain setup as E1, but every
model parameter is randomly initialized. No official Union14M checkpoint
and no D2 checkpoint are loaded.
...
for session in "$TRAIN_SESSION" "$WATCH_SESSION"; do
  if tmux has-session -t "$session" 2>/dev/null; then
    if [[ "$REPLACE" == "1" ]]; then
      ...
      echo "Use --replace to kill and restart E0 sessions." >&2
      exit 1
    fi
  fi
done
...
if [[ "$REPLACE" == "1" ]]; then
  rm -rf \
    "$RUN_MODEL_DIR" \
    "$EVAL_ROOT"/e0_random_target_dev_epoch_* \
    "$EVAL_ROOT/e0_random_target_best_target_dev" \
    "$EVAL_ROOT/e0_random_target_best_target_test" \
    "$EVAL_ROOT/e0_random_target_final_summary.json"
fi
...
echo "[1/3] Preparing E0 random-init target-domain labels and config..."
python scripts/svtrv2/prepare_e0_target_only.py \
  --root "$ROOT" \
  --max-epoch "$MAX_EPOCH" \
...
if grep -Eq 'pretrained_model:[[:space:]]*[^[:space:]]+' "$CONFIG_PATH"; then
  echo "E0 safety check failed: pretrained_model is not empty." >&2
  grep 'pretrained_model:' "$CONFIG_PATH" >&2
  exit 1
fi
if grep -Eq 'checkpoints:[[:space:]]*[^[:space:]]+' "$CONFIG_PATH"; then
  echo "E0 safety check failed: checkpoints is not empty." >&2
  grep 'checkpoints:' "$CONFIG_PATH" >&2
  exit 1
fi
...
for EPOCH in \$(seq "$EVAL_EVERY" "$EVAL_EVERY" "$MAX_EPOCH"); do
  ...
  OUT_DIR="$EVAL_ROOT/e0_random_target_dev_epoch_\${EPOCH_PAD}"
...
rm -rf "$EVAL_ROOT/e0_random_target_best_target_dev"
python scripts/svtrv2/evaluate_d2_checkpoint.py \
  --root "$ROOT" \
  --config "$CONFIG_PATH" \
  ...
  --labels-dir "$LABELS_DIR" \
  --label-prefix target \
  --output-dir "$EVAL_ROOT/e0_random_target_best_target_dev" \
  --device-id "$EVAL_GPU" \
  --batch-size 32 \
  --require-python-bidi \
...
rm -rf "$EVAL_ROOT/e0_random_target_best_target_test"
python scripts/svtrv2/evaluate_d2_checkpoint.py \
  --root "$ROOT" \
  --config "$CONFIG_PATH" \
  --checkpoint "\$BEST_CKPT" \
  ...
  --labels-dir "$LABELS_DIR" \
  --label-prefix target \
  --output-dir "$EVAL_ROOT/e0_random_target_best_target_test" \
  --device-id "$EVAL_GPU" \
  --batch-size 32 \
  --require-python-bidi \
  2>&1 | tee "$LOG_DIR/final_test_eval.log"

python -c "import json, pathlib; out=pathlib.Path('$EVAL_ROOT/e0_random_target_final_summary.json'); best=json.load(open('$RUN_MODEL_DIR/best_target_dev_macro_cer.json', encoding='utf-8')); dev=json.load(open('$EVAL_ROOT/e0_random_target_best_target_dev/metrics_macro_summary.json', encoding='utf-8')); test=json.load(open('$EVAL_ROOT/e0_random_target_best_target_test/metrics_macro_summary.json', encoding='utf-8')); data={'experiment':'E0_random_initialization_target_domain_only','initialization':'random','best':best,'dev':dev,'test':test,'logs':'$LOG_DIR','run_dir':'$RUN_MODEL_DIR'}; out.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding='utf-8'); print(json.dumps(data, ensure_ascii=False, indent=2))" \
  2>&1 | tee "$LOG_DIR/final_summary.log"

echo "DONE"
echo "Best checkpoint: \$BEST_CKPT"
echo "Final summary: $EVAL_ROOT/e0_random_target_final_summary.json"
EOF
